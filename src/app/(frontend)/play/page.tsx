...
export default function PlayPage() {
    ...
    const formatCards = useMemo(() => {
        const result: { [key: string]: CardType } = {};
        cards.data?.forEach((c) => (result[c._id] = c));
        return result;
    }, [cards.data]);
    ...
    useEffect(() => {
        if (!id && cards.data) {
            ...
            const newId = cards.data[randomIndx]._id;
    ...
    return (
        <main className="mx-auto max-w-[600px] text-base px-5 md:px-10 min-h-screen flex flex-col justify-between gap-y-4">
            ...
            <div
                className={
                    "flex w-full justify-between items-center py-4 px-6 sticky bottom-0"
                }
            >
                <Button
                    variant={"game"}
                    ...
                    onClick={() => {
                        if (!skippedId) {
                            GameStore.setSkippedCard(card._id);
                            GameStore.setNextCard(
                                card._id,
                                cards.data ?? [],
                                true
                            );
                        } else if (skippedId && skippedId !== card._id) {
                            GameStore.setPreviousId(card._id);
                ...
                <Button
                    ...
                    onClick={() => {
                        if (skippedId === card._id && previousId) {
                            GameStore.setCurrentId(previousId);
                            GameStore.addTurnHistory(card._id);
                            GameStore.setSkippedCard("Done");
                        } else {
                            GameStore.setNextCard(card._id, cards.data ?? []);
                        }
                    }}
                >
                    <ChevronRight />
